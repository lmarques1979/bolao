...
class TimeControllerSpec extends Specification {
    ...
    void "Test the index action returns the correct model"() {
        ...
        then:"The model is correct"
            !model.timeInstanceList
            model.timeInstanceCount == 0
    }
    ...
    void "Test the update action performs an update on a valid domain instance"() {
        ...
        when:"A valid domain instance is passed to the update action"
            response.reset()
            populateValidParams(params)
            time = new Time(params).save(flush: true)
            controller.update(time)

        then:"A redirect is issues to the show action"
            response.redirectedUrl == "/time/show/$time.id"
            flash.message != null
    }
    ...
    void "Test that the delete action deletes an instance if it exists"() {
        ...
        when:"A domain instance is created"
            response.reset()
            populateValidParams(params)
            def time = new Time(params).save(flush: true)
